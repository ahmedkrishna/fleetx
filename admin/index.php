    ...
    <table class="admin-table">
      ...
      <tbody>
        <?php foreach ($recent_auctions as $auc):
          ...
          $admin_thumb = fleetx_vehicle_thumb($auc['image_url'] ?? '', $admin_thumb_id, $admin_thumb_type, $auc['make'] ?? '');
        ?>
          <tr>
            <td style="font-family:var(--font-en);color:var(--gray-500)">#AUC-<?php echo sprintf("%03d", $auc['id']); ?></td>
            <td>
              <div style="display:flex;align-items:center;gap:var(--space-3)">
                <img src="<?= htmlspecialchars($admin_thumb['src']) ?>" alt="" loading="lazy" decoding="async" onerror="<?= $admin_thumb['onerror'] ?>" style="width:48px;height:36px;object-fit:cover;border-radius:var(--radius-sm);background:#f1f5f9">
                <div>
