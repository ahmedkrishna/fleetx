...
if ($db_connected) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['create_auction'])) {
        ...
        $stmt_auc = $conn->prepare("INSERT INTO auctions (vehicle_id, seller_id, title, type, start_price, reserve_price, current_price, min_increment, start_time, end_time, status, is_featured) VALUES (?, 2, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt_auc->bind_param("isssddddsssi", $vehicle_id, $title, $type, $start_price, $reserve_price, $start_price, $min_increment, $start_time, $end_time, $status, $is_featured);
...
$auctions_list = [];
if ($db_connected) {
    $sql_all = "SELECT a.*, v.make, v.model, v.year, v.mileage, v.city, v.image_url, u.name AS seller_name,
...
// Fallback Mock Data if database offline
...
<head>
  ...
  <title>إدارة المزادات | لوحة الإدارة | مزادي Mazadi</title>
...
<aside class="admin-sidebar" id="admin-sidebar" role="complementary" aria-label="القائمة الجانبية">
  ...
  <nav class="admin-nav" role="navigation" aria-label="قائمة الإدارة">
    ...
    <a href="auctions.php" class="admin-nav-link active" id="nav-auctions">
