...
function initCardNavigation() {
  document.querySelectorAll('.auction-card[data-id]').forEach(card => {
    card.addEventListener('click', (e) => {
      ...
      window.location.href = `/auction-live.php?id=${id}`;
    });
...
/* ── Global Favorite Toggle (Cookies & UI) ──────────────── */
...
window.toggleFavorite = function(id, btn) {
    if (!id) return;
    let favs = getCookie('favorites');
    let arr = favs ? favs.split(',') : [];
    let isFav = false;
    
    if (arr.includes(id.toString())) {
        arr = arr.filter(x => x !== id.toString());
        isFav = false;
    } else {
    ...
    setCookie('favorites', arr.join(','), 30);
    
    // Update UI for the button
...
function syncFavoritesUI() {
    let favs = getCookie('favorites');
    let arr = favs ? favs.split(',') : [];
    document.querySelectorAll('.card-fav, .btn-outline[onclick^="toggleFavorite"]').forEach(btn => {
        let onclickStr = btn.getAttribute('onclick');
        if (onclickStr) {
            let match = onclickStr.match(/toggleFavorite\(([\d]+)/);
            if (match && arr.includes(match[1])) {
                let icon = btn.querySelector('i');
                if (icon) {
                    icon.classList.remove('ph');
                    icon.classList.add('ph-fill');
